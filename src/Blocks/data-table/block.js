...
registerBlockType( 'wilson-api/data-table', {
	...
	description: __(
		'Display data from Wilson API in a table format',
		'wilson-api-plugin'
	),
	category: 'widgets',
	icon: 'list-view',
	keywords: [
		__( 'api', 'wilson-api-plugin' ),
		__( 'data', 'wilson-api-plugin' ),
		__( 'table', 'wilson-api-plugin' ),
	],
	attributes: {
		visibleColumns: {
			type: 'object',
	...
	edit: ( { attributes, setAttributes } ) => {
		...
		useEffect( () => {
			if ( ! blockId ) {
				setAttributes( {
					blockId: `wilson-api-block-${ Date.now() }-${ Math.random()
						.toString( 36 )
						.substr( 2, 9 ) }`,
				} );
			}
		...
		const fetchData = async () => {
			...
			try {
				const response = await fetch(
					`${ wilsonApiBlock.ajaxUrl }?action=${ wilsonApiBlock.action }`,
					{
						method: 'GET',
						headers: {
				...
				if ( ! response.ok ) {
					throw new Error(
						__( 'Failed to fetch data', 'wilson-api-plugin' )
					);
				}
				...
				if ( ! result.success ) {
					throw new Error(
						result.data?.message ||
							__( 'Unknown error', 'wilson-api-plugin' )
					);
				}
		...
		const renderTable = () => {
			if ( ! data ) {
				return (
					<p>{ __( 'No data available', 'wilson-api-plugin' ) }</p>
				);
			}
				...
				if ( visibleCols.length === 0 ) {
					return (
						<Notice status="warning" isDismissible={ false }>
							{ __(
								'Please enable at least one column in the block settings.',
								'wilson-api-plugin'
							) }
						</Notice>
					);
		...
		return (
			<>
				<InspectorControls>
					<PanelBody
						title={ __( 'Display Settings', 'wilson-api-plugin' ) }
					>
						<ToggleControl
							label={ __(
								'Show Table Header',
					...
					{ columns.length > 0 && (
						...
						>
							<p className="components-base-control__help">
								{ __(
									'Toggle which columns to display in the table',
									'wilson-api-plugin'
								) }
							</p>
							{ columns.map( ( col ) => (
				...
				<div { ...blockProps }>
					<div className="wilson-api-block-header">
						...
						>
							{ loading
								? __( 'Refreshing...', 'wilson-api-plugin' )
								: __( 'Refresh', 'wilson-api-plugin' ) }
						</button>
					</div>
					...
					{ loading && (
						<div className="wilson-api-block-loading">
							<Spinner />
							<p>
								{ __( 'Loading data...', 'wilson-api-plugin' ) }
							</p>
						</div>
					) }
